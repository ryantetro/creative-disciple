...
export default function ReadingPlanDisplay({ summary, plan }: ReadingPlanDisplayProps) {
    ...
    return (
        <div className="bg-white rounded-xl p-4 sm:p-6 shadow-md border border-slate-200">
            ...
            {/* Summary */}
            <div className="mb-6 sm:mb-8 p-4 sm:p-5 bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg border border-blue-200">
                <p className="text-sm sm:text-base text-[#1F2937] leading-relaxed">{summary}</p>
            </div>

            {/* Progress */}
            <div className="mb-6 sm:mb-8">
                <div className="flex justify-between items-center mb-2">
                    <span className="text-xs sm:text-sm font-semibold text-[#1F2937]">Progress</span>
                    <span className="text-xs sm:text-sm font-bold text-[#5E72E4]">{progress}%</span>
                </div>
            ...
            <div className="space-y-4 sm:space-y-6">
                {weeks.map((week, weekIndex) => (
                    <div key={weekIndex} className="bg-slate-50 rounded-lg p-4 sm:p-5 border border-slate-200">
                        <h3 className="text-base sm:text-lg font-semibold text-[#1F2937] mb-3 sm:mb-4">
                            Week {weekIndex + 1}
                        </h3>
                                        ...
                                        <div className="flex items-start gap-2 sm:gap-3">
                                            {!isRestDay && (
                                                <input
                                                    ...
                                                    className="mt-1 w-5 h-5 sm:w-5 sm:h-5 text-[#5E72E4] rounded cursor-pointer
                                                     focus:ring-2 focus:ring-[#5E72E4] focus:ring-offset-2 min-w-[20px] min-h-[20px]"
                                                />
                                            )}
                                            <div className="flex-1">
                                                ...
                                                {!isRestDay && day.chapters.length > 0 && (
                                                    <div className="text-xs sm:text-sm text-[#5E72E4] font-medium mb-1 sm:mb-2">
                                                        {day.chapters.join(", ")}
                                                    </div>
                                                )}
